...
const MATCHES = [
  ...
  {id:3,person:"Sam Torres",avatar:"ST",asset:"Shopify Plugin",type:"Digital Product / SaaS",status:"posted",due:"Feb 28",channels:["Shopify App Store"],color:"#F4A832",postedCh:"Shopify App Store"},
  {id:4,person:"Priya Nair",avatar:"PN",asset:"Brand Strategy Session",type:"Advisory / Consulting Skills",status:"posted",due:"Feb 20",channels:["Google Business Profile","Yelp"],color:"#6B4226",postedCh:"Google Business Profile"},
...
function Card({children,sx={},hover=true,dim=false}){
  ...
  return <div onMouseEnter={()=>hover&&sH(true)} onMouseLeave={()=>hover&&sH(false)}
    style={{background:dim?"#F7F2ED":"#fff",borderRadius:20,border:`1.5px solid ${h&&!dim?T.orangeP:"#F0E8E0"}`,
    boxShadow:h&&!dim?"0 8px 32px rgba(61,43,31,0.10)":"0 2px 10px rgba(61,43,31,0.06)",
    transition:"all 0.22s",transform:h&&!dim?"translateY(-2px)":"none",opacity:dim?0.75:1,...sx}}>{children}</div>;
}
      ...
      <div style={{background:"#fff",borderRadius:28,padding:"40px 44px",maxWidth:500,width:"100%",boxShadow:"0 24px 80px rgba(61,43,31,0.3)",position:"relative",animation:"popIn 0.25s ease"}}>
        ...
        {step==="form"?(
          <>
            ...
            <Btn onClick={submit} sx={{width:"100%",justifyContent:"center",marginTop:8}}>{loading?"Sending...":"✦ Request My Spot →"}</Btn>
          </>
        ):(
                  ...
                  <div style={{display:"flex",gap:14,alignItems:"flex-start",marginBottom:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:8,marginTop:5}}>
                        ...
                        <span style={{fontSize:12,color:T.brownL,fontFamily:"'DM Sans',sans-serif"}}>{m.rating} avg · {m.exchanges} exchanges</span>
                      </div>
                    </div>
                  </div>
        ...
        {tab==="matches"&&(
          <div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              {MATCHES.map(m=>{const s=sc[m.status];return(
                <Card key={m.id} sx={{padding:"20px 24px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:16}}>
                    ...
                    <div style={{display:"flex",gap:8,flexShrink:0}}>
                      ...
                      {m.status==="posted"&&<div style={{display:"flex",alignItems:"center",gap:6,fontSize:13,color:T.green,fontFamily:"'DM Sans',sans-serif",fontWeight:600}}>✓ {m.postedCh}</div>}
...
function AssetPage(){
  ...
  const[a,setA]=useState({name:"",url:"",type:"",desc:"",channels:[],fbTypes:[],reqStars:false,reqTwo:false,forClient:false,clientName:""});
